...
def load_and_freeze_mlp(model_path):
    ...
    try:
        ...
        print(f"Error inferring MLP architecture: {e}")
        return None, None

    print(f"MLP: input_dim={input_dim}, hidden_width={hidden_width}, hidden_depth={hidden_depth}")
    ...
    model.output_layer = nn.Identity()
    
    for param in model.parameters():
        param.requires_grad = False

    print(f"MLP loaded and frozen. Penultimate features: {hidden_width}-dim")
    return model, hidden_width
...
def load_and_freeze_cnn(model_path):
    ...
    try:
        ...
        print(f"Error inferring CNN parameters: {e}")
        return None

    print(f"CNN: num_classes={original_num_classes}, dropout={original_dropout_rate}")
    ...
    print("CNN loaded and frozen. Penultimate features: 1280-dim")
    return model
...
class FeatureFusionModel(nn.Module):
    ...
    def __init__(self, cnn_model_path, mlp_model_path, 
        ...
        self.cnn_extractor = load_and_freeze_cnn(cnn_model_path)
        self.mlp_extractor, mlp_feature_dim = load_and_freeze_mlp(mlp_model_path)
        
        self.mlp_projection = nn.Sequential(
            nn.Linear(mlp_feature_dim, 1280),
            nn.ReLU(),
            nn.BatchNorm1d(1280),
    ...
    def forward(self, x_spectrogram, x_librosa):
        ...
        mlp_projected = self.mlp_projection(mlp_features) 
        
        fused_features = torch.cat([cnn_features, mlp_projected], dim=1)
...
def train_final_model(best_result, config):
    ...
    print("\n" + "="*80)
    print("FINAL MODEL TRAINING (all train2 data use)")
...
def main():
    BASE_DATA_DIR = '/home/UG/n2402401d/SC4001_Project/FINAL/data'
    MODEL_SAVE_DIR = '/home/UG/n2402401d/SC4001_Project/FINAL/feature_level_fusion'
    ...
    CNN_PATH = '/home/UG/n2402401d/SC4001_Project/FINAL/cnn_cv/best_cnn_phase1.pth'
    MLP_PATH = '/home/UG/n2402401d/SC4001_Project/FINAL/mlp_cv/final_mlp_model.pth'
